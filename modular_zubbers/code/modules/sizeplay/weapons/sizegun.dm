...
/obj/projectile/beam/sizeray/on_hit(atom/target, blocked = 0, pierce_hit)
	. = ..()
	if(. != BULLET_ACT_HIT)
		return FALSE
	...
	var/mob/living/carbon/mob_to_resize = target
	if(!istype(mob_to_resize))
		return FALSE

	// Add pref checking here.
	...
	mob_to_resize.set_size(resize_to)
